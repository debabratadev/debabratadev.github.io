function createShootingGame(gameConfig, enableSound) {
  const correct_audio =
    "data:audio/wav;base64,/+MYxAAMyRqsAUp4AP////88888wwwwwwwww888889BAAKCbi5lzUc7xWKxkZ379+/j3B8EAQ/DAPh///lAfD6obt2/N/mUr/+MYxAcOMi68AYKgAH/WYlIiwbN6JLeo1AOAAsrMkki8Yl0GigFGSfwQkhSRn8uqS6S0f///6jIu ";
  const incorrect_audio =
    "data:audio/wav;base64,";

  var canvas,
    ctx,
    $title,
    aims = [], // Targets for a level
    score = 0,
    level = 0,
    ratio = window.devicePixelRatio, // Pixel ratio of the screen
    normalSpeed = 1, // Normal speed for moving targets
    fastSpeed = 3, // Fast speed for moving targets
    correctSound = new Audio(correct_audio),
    inCorrectSound = new Audio(incorrect_audio),
    rotateTargets = false, //Flag to rotate targets
    inCircle = false; //Flag to draw in circle

  function Aim() {
    this.x = 0;
    this.y = 0;
    this.radius = 30;
    this.speed = normalSpeed;
    this.animationDirection = "horizontal";
    this.animate = false;
    this.radian = (-0.5 * Math.PI) / 180;
    this.sin = Math.sin(this.radian);
    this.cos = Math.cos(this.radian);
    this.hit = false;

    this.draw = function () {
      //Draw outer Red.
      var prevFill = ctx.fillStyle;
      ctx.beginPath();
      ctx.fillStyle = "#FB0007";
      ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI);
      ctx.fill();

      //Draw Middle white.
      ctx.beginPath();
      ctx.fillStyle = "#EAEAEA";
      ctx.arc(this.x, this.y, this.radius - 10, 0, 2 * Math.PI);
      ctx.fill();

      //Draw Inner Red.
      ctx.beginPath();
      ctx.fillStyle = "#BD0004";
      ctx.arc(this.x, this.y, this.radius - 20, 0, 2 * Math.PI);
      ctx.fill();
      ctx.closePath();
      ctx.fillStyle = prevFill;
    };

    //Check if Mouse hit the target
    this.hitTest = function (mouse) {
      ctx.beginPath();
      ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI);
      if (ctx.isPointInPath(mouse.x * ratio, mouse.y * ratio)) {
        ctx.closePath();
        return true;
      } else {
        return false;
      }
    };
    this.contact = function (mouse) {
      //If already hit then return.
      if (this.hit) {
        return;
      }

      var da = mouse.x - this.x;
      var db = mouse.y - this.y;

      var distance = Math.sqrt(da * da + db * db);

      //Set Score based on hit position.
      if (distance <= 10) {
        score += 5;
      } else if (distance <= 20) {
        score += 3;
      } else if (distance <= 10) {
        score += 1;
      }

      console.log(score);
      this.hit = true;
      shootingGame.checkResult();
    };

    //It animates in horizontal or vertical direction.
    this.move = function () {
      if (!this.animate) {
        return;
      }

      var canvasWidth = canvas.width / ratio,
        canvasHeight = canvas.height / ratio;

      if (this.animationDirection == "horizontal") {
        //Reached right edge
        if (this.x + this.radius >= canvasWidth) {
          this.speed = -this.speed;
        }
        //Reached left edge
        if (this.x - this.radius <= 0) {
          this.speed = Math.abs(this.speed);
        }
        this.x += this.speed;
      } else if (this.animationDirection == "vertical") {
        //Reached Bottom
        if (this.y + this.radius >= canvasHeight) {
          this.speed = -this.speed;
        }
        //Reached Top
        if (this.y - this.radius <= 0) {
          this.speed = Math.abs(this.speed);
        }
        this.y += this.speed;
      }
    };

    //Rotate Target
    this.rotate = function () {
      //Run When enabled
      if (!rotateTargets) {
        return;
      }

      //Rotate around the center
      var cx = canvas.width / ratio / 2,
        cy = canvas.height / ratio / 2;

      var x = this.x - cx,
        y = cy - this.y;
      var x1 = x * this.cos - y * this.sin;
      var y1 = y * this.cos + x * this.sin;
      this.x = cx + x1;
      this.y = cy - y1;
    };
  }

  function drawInCircle() {
    if (!inCircle) {
      return;
    }
    var pi2 = 2 * Math.PI,
      aim = new Aim(),
      radius = canvas.width / ratio / 2 - aim.radius,
      segments = parseInt((pi2 * radius) / (2 * aim.radius)), // Find total targets required based on perimeter
      segs = pi2 / segments,
      i = 0,
      ax,
      ay,
      x = canvas.width / ratio / 2,
      y = canvas.height / ratio / 2,
      index = 0;

    ctx.save();
    ctx.translate(x, y);
    ctx.translate(-x, -y);

    //Position all targets in a circle
    for (; i < pi2; i += segs) {
      ax = x + radius * Math.cos(i);
      ay = y + radius * Math.sin(i);
      var aim = aims[index];
      if (!aim) {
        break;
      }
      aim.x = ax;
      aim.y = ay;
      index++;
    }

    ctx.restore();
  }

  var shootingGame = {
    setupHtml: function () {
      var $gameContainer = $("<div>")
        .css({
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          flexDirection: "column",
          height: "100vh",
        })
        .appendTo(document.body);

      var $canvas = $("<canvas>")
        .css({
          height: "100%",
          width: "100%",
          border: "2px solid #ddd",
        })
        .appendTo($gameContainer);

      canvas = $canvas[0];
      ctx = canvas.getContext("2d");

      $title = $("<div>")
        .css({
          textAlign: "center",
          paddingTop: "10px",
        })
        .appendTo($gameContainer);

      function updateCanvasWidth() {
        var windowWidth = $(window).width();
        if (windowWidth <= 620) {
          $gameContainer.css({ padding: "0 9px" });
          var canvasWidth = windowWidth - 18;
          canvas.width = canvasWidth * ratio;
          canvas.height = canvasWidth * ratio;
          $canvas.css({
            height: canvasWidth + "px",
            width: canvasWidth + "px",
          });
        } else {
          $gameContainer.css({ padding: 0 });
          canvas.width = 600 * ratio;
          canvas.height = 600 * ratio;
          $canvas.css({ height: "600px", width: "600px" });
        }
        ctx.scale(ratio, ratio);
      }

      $(window).on("resize", function () {
        updateCanvasWidth();
        shootingGame.nextLevel(--level);
      });

      updateCanvasWidth();
    },
    createAim(x, y) {
      var aim = new Aim();
      aim.x = x;
      aim.y = y;
      aims.push(aim);

      return aim;
    },
    drawOneTarget: function (animate, speed) {
      var aim = shootingGame.createAim(
        canvas.width / ratio / 2,
        canvas.height / ratio / 2
      );
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }
    },
    drawThreeTarget: function (animate, speed) {
      var height = canvas.height / ratio,
        halfHeight = height / 2;

      //Top
      var aim = shootingGame.createAim(halfHeight, halfHeight - halfHeight / 2);
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }

      //Middle
      aim = shootingGame.createAim(halfHeight, halfHeight);
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }

      //Bottom
      aim = shootingGame.createAim(halfHeight, halfHeight + halfHeight / 2);
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }
    },
    drawLongDistanceStaticTargets: function (animate, speed) {
      var canvasWidth = canvas.width / ratio,
        canvasHeight = canvas.height / ratio;
      var aim = new Aim();

      //Top Left.
      aim = shootingGame.createAim(aim.radius, aim.radius);
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }

      //Top Right
      aim = shootingGame.createAim(canvasWidth - aim.radius, aim.radius);
      if (animate) {
        aim.animate = true;
        aim.animationDirection = "vertical";
        aim.speed = speed;
      }

      //Bottom Left.
      aim = shootingGame.createAim(aim.radius, canvasHeight - aim.radius);
      if (animate) {
        aim.animate = true;
        aim.animationDirection = "vertical";
        aim.speed = speed;
      }

      //Bottom Right
      aim = shootingGame.createAim(
        canvasWidth - aim.radius,
        canvasHeight - aim.radius
      );
      if (animate) {
        aim.animate = true;
        aim.speed = speed;
      }
    },
    oneStaticTarget: function () {
      shootingGame.drawOneTarget(false);
    },
    twoStaticTargets: function () {
      var height = canvas.height / ratio,
        halfHeight = height / 2;
      shootingGame.createAim(halfHeight, halfHeight - halfHeight / 2);

      shootingGame.createAim(halfHeight, halfHeight + halfHeight / 2);
    },
    oneMovingTarget: function () {
      shootingGame.drawOneTarget(true, normalSpeed);
    },
    threeMovingTargets: function () {
      shootingGame.drawThreeTarget(true, normalSpeed);
    },
    oneFastMovingTarget: function () {
      shootingGame.drawOneTarget(true, fastSpeed);
    },
    threeFastMovingTargets: function () {
      shootingGame.drawThreeTarget(true, fastSpeed);
    },
    longDistanceStaticTargets: function () {
      shootingGame.drawLongDistanceStaticTargets(false);
    },
    movingLongDistanceTargets: function () {
      shootingGame.drawLongDistanceStaticTargets(true, normalSpeed);
    },
    horizontalLineShapedStaticTargets: function () {
      var halfHeight = canvas.height / ratio / 2,
        aim = new Aim(),
        size = parseInt(halfHeight / aim.radius),
        pad = halfHeight - size * aim.radius,
        x = pad + aim.radius; //Center horizontally

      for (let index = 0; index < size; index++) {
        shootingGame.createAim(x, halfHeight);
        x += 2 * aim.radius;
      }
    },
    verticalLineShapedStaticTargets: function () {
      var halfWidth = canvas.width / ratio / 2,
        aim = new Aim(),
        size = parseInt(halfWidth / aim.radius),
        pad = halfWidth - size * aim.radius,
        y = pad + aim.radius; //Center vertically

      for (let index = 0; index < size; index++) {
        shootingGame.createAim(halfWidth, y);
        y += 2 * aim.radius;
      }
    },
    circleShapedStaticTargets: function () {
      inCircle = true;
      var halfWidth = canvas.width / ratio / 2,
        aim = new Aim(),
        y = aim.radius;

      var pi2 = 2 * Math.PI,
        radius = halfWidth - aim.radius,
        segments = parseInt((pi2 * radius) / (2 * aim.radius)); //Total targets require based on perimeter

      for (let index = 0; index < segments; index++) {
        shootingGame.createAim(halfWidth, y);
        y += 2 * aim.radius;
      }
    },
    rotatingTargets: function () {
      shootingGame.verticalLineShapedStaticTargets();
      rotateTargets = true;
    },
    fullStackOfTargets: function () {
      var halfWidth = canvas.width / ratio / 2,
        aim = new Aim(),
        size = parseInt(halfWidth / aim.radius), //Number of rows/columns
        pad = halfWidth - size * aim.radius,
        x = pad + aim.radius, //Center align
        y = pad + aim.radius; //Center align

      for (let index = 0; index < size; index++) {
        for (let j = 0; j < size; j++) {
          shootingGame.createAim(x, y);
          x += 2 * aim.radius;
        }
        y += 2 * aim.radius;
        x = pad + aim.radius;
      }
    },
    nextLevel: function () {
      if (level >= 13) {
        gameover(score);
        return;
      }
      aims = [];
      rotateTargets = false;
      inCircle = false;
      shootingGame[gameConfig.levels[level].name]();
      $title.html(gameConfig.levels[level].title);
      level++;
    },
    bindEvents: function () {
      var offsetLeft = 0,
        offsetTop = 0,
        canvasClone = canvas,
        mouse = {};

      function updateOffset() {
        //Find the canvas left and top offeset
        do {
          offsetLeft += canvasClone.offsetLeft;
          offsetTop += canvasClone.offsetTop;
        } while ((canvasClone = canvasClone.offsetParent));
      }

      updateOffset();

      //Get relative mouse position of Canvas
      function updateMouseXY(e) {
        mouse.x = e.pageX - offsetLeft;
        mouse.y = e.pageY - offsetTop;
      }

      function onMouseDown() {
        var hitTarget = false;
        for (var i = 0; i < aims.length; i++) {
          if (aims[i].hitTest(mouse)) {
            aims[i].contact(mouse);
            hitTarget = true;
          }
        }

        if (hitTarget) {
          enableSound && correctSound.play();
        } else {
          enableSound && inCorrectSound.play();
        }
      }

      canvas.addEventListener("mousedown", function (e) {
        updateMouseXY(e);
        onMouseDown();
      });

      canvas.addEventListener("touchstart", function (e) {
        e.preventDefault();
        updateMouseXY(e.touches[0]);
        onMouseDown();
      });

      $(window).on("resize", function () {
        canvasClone = canvas;
        offsetLeft = 0;
        offsetTop = 0;
        updateOffset();
      });
    },
    checkResult: function () {
      var targets = aims.filter((aim) => !aim.hit);

      //Go to next level only when all targets were hit.
      if (targets.length === 0) {
        shootingGame.nextLevel();
      }
    },
    init: function () {
      shootingGame.setupHtml();
      shootingGame.bindEvents();
      shootingGame.nextLevel();
    },
  };

  shootingGame.init();
  (function drawFrame() {
    window.requestAnimationFrame(drawFrame, canvas);
    ctx.clearRect(0, 0, canvas.width, canvas.height); //Clear canvas
    drawInCircle();
    aims.forEach(function (aim) {
      aim.move();
      aim.rotate();
    });
    aims.forEach(function (aim) {
      aim.draw();
    });
  })();
}

$(function () {
  var gameConfigObj = {
    levels: [
      {
        name: "oneStaticTarget",
        title: "1 static target",
      },
      {
        name: "twoStaticTargets",
        title: "2 static targets",
      },
      {
        name: "oneMovingTarget",
        title: "1 moving target",
      },
      {
        name: "threeMovingTargets",
        title: "3 moving targets",
      },
      {
        name: "oneFastMovingTarget",
        title: "1 fast moving target",
      },
      {
        name: "threeFastMovingTargets",
        title: "3 fast moving targets",
      },
      {
        name: "longDistanceStaticTargets",
        title: "Long distance static targets",
      },
      {
        name: "movingLongDistanceTargets",
        title: "Moving long distance targets",
      },
      {
        name: "horizontalLineShapedStaticTargets",
        title: "Horizontal line shaped static targets",
      },
      {
        name: "verticalLineShapedStaticTargets",
        title: "Vertical line shaped static targets",
      },
      {
        name: "circleShapedStaticTargets",
        title: "Circle shaped static targets",
      },
      {
        name: "rotatingTargets",
        title: "Rotating targets",
      },
      {
        name: "fullStackOfTargets",
        title: "Full stack of targets",
      },
    ],
  };
  createShootingGame(gameConfigObj, true);
});

function gameover(level) {
  alert(level);
}
