function createColorGame(levelConfig, enableSound) {
  ...
  var $box, $content, $pauseContent, $score, $timer;
  ...
  var colorGame = {
    nextLevel: function (level) {
      ...
      for (let index = 0; index < items; index++) {
        colorGame.createItem(index, width, colors);
      }
    },
    ...
    createItem: function (index, width, colors) {
      ...
      if (index == correctPosition) {
        $span.css({ backgroundColor: colors[1] });
      }

      //Bind click event to calculate result
      $span.click(colorGame.calculateResult);
      return $span;
    },
    ...
    pauseGame: function () {
      ...
      $pauseContent.css({ display: "block" });
      gamePaused = true;
    },
    resumeGame: function () {
    ...
    setCounter() {
      ...
      intervalId = setInterval(() => {
        ...
        //Show in red when timer reaches 5.
        if (timer == 5) {
          $timer.css({ color: "red", backgroundColor: "#fff" });
        }
    ...
    setUpHtml: function () {
      ...
      var $scoreText = $("<span>").html("Score: ").appendTo($scoreContainer);
      $score = $("<span>").appendTo($scoreContainer);
      ...
      $pauseBtn.click(colorGame.pauseGame);
      ...
      $boxContainer.addClass("patra-color");
      ...
      $resumeButton.click(colorGame.resumeGame);

      //Run media queries to make it responsive
      function runMediaQuery(x) {
        if (x.matches) {
          // If media query matches
          $gameContainer.css({ padding: 0 });
          $content.css({ margin: "0px", maxWidth: "600px" });
          $boxContainer.css({ height: "600px", width: "600px" });
        } else {
          $gameContainer.css({ padding: "0 9px" });
          $boxContainer.css({ height: "100%", width: "100%" });
        }
      }
